...
const experience: ExperienceWithDuration<ExperienceOrMore> = {
  ...
  angular: {
    start: moment("1.6.2015", "DD.MM.YYYY"),
    end: moment("1.1.2021", "DD.MM.YYYY"),
  },
  arch: { start: moment("1.1.2020", "DD.MM.YYYY"), end: moment() },
  aws: { start: moment("1.1.2021", "DD.MM.YYYY"), end: moment() },
  azure: { start: moment("1.10.2018", "DD.MM.YYYY"), end: moment() },
  build: { start: moment("1.9.2014", "DD.MM.YYYY"), end: moment() },
  python: { start: moment("1.9.2012", "DD.MM.YYYY"), end: moment() },
  ...
  be: { start: moment("1.9.2013", "DD.MM.YYYY"), end: moment() },
  fe: { start: moment("1.9.2009", "DD.MM.YYYY"), end: moment() },
};
  ...
  for (const key in experience) {
    ...
    if (Array.isArray(ex)) {
      const totalInYears = ex.reduce((total, exp) => {
        total += Math.round(moment.duration(exp.end.diff(exp.start)).asYears());
        return total;
      }, 0);
